...
- 기본값은 디자인 기준값으로 두되, 실제 렌더링 값은 반응형 레이어가 화면 너비/높이에 맞게 자동 보정한다.

### 화면 레이아웃

- 좌우 기본 패딩 base는 `24`
- compact 화면에서는 좌우 패딩을 `16~20`까지 줄일 수 있다
- 상단 Safe Area 아래 첫 요소 시작 간격 base는 `20`
- 제목과 설명 간격 base는 `8`
- 주요 섹션 간 간격 base는 `24`
- 카드 내부 패딩은 `20` 또는 `24`를 기준으로 하되, 작은 화면에서는 축소 가능해야 한다
- 하단 고정 버튼 영역은 `24` 패딩 + `56` 높이 기준이며, 작은 화면에서는 버튼 높이와 상하 여백을 함께 줄인다
...
- `lib/shared/layout/app_responsive.dart`
...
- 화면 너비/높이를 compact / regular / expanded 성격으로 해석
- 이미지 높이, 버튼 높이, 폰트 크기, 카드 높이를 clamp 기반으로 보정
- `BuildContext`에서 공통 helper로 접근할 수 있게 제공
...
- 화면비율 대응은 화면별 임시 분기보다 `AppResponsive` 공통 규칙을 우선 사용한다.
- 작은 화면에서 정보가 다 안 들어오면 overflow 대신 스크롤로 안전하게 처리한다.
- 넓은 화면에서 1열이 과하게 늘어나면 max width 또는 column 분기를 사용한다.
...
현재 `frontend_v3`는 아래 3개 구간을 기준으로 QA한다.
...
- 대표 상황:
  - 작은 Android 기기
  - 키보드가 올라온 상태
  - 화면 상단/하단에 고정 UI가 많은 상태
- 대응 원칙:
  - 캐릭터 이미지와 카드 높이부터 먼저 줄인다
  - CTA 높이와 섹션 간격도 함께 줄인다
  - 그래도 안 들어오면 내부 스크롤을 허용한다
  - overflow는 허용하지 않는다
...
- 대응 원칙:
  - 디자인 기준값을 최대한 유지한다
  - 요소 간 간격이 너무 조밀하거나 너무 비지 않게 한다
  - 하단 CTA와 본문 카드의 관계가 안정적으로 보여야 한다
...
- 예시 기준:
  ...
  - 또는 웹/태블릿 레이아웃
- 대응 원칙:
  - 콘텐츠는 max width 안에 묶는다
  - 필요한 구간만 2열 또는 넓은 grid로 확장한다
  - 말풍선, 홈 인사, 상품 카드가 좌우로 과하게 길어지지 않게 한다

## 반응형 QA 체크리스트

### 전 화면 공통

- 화면 진입 직후 `RenderFlex overflow`가 없어야 한다
- 하단 CTA 또는 마이크 바가 본문 핵심 정보를 과하게 가리지 않아야 한다
- Safe Area 안에서 상단 버튼, 단계바, 제목이 잘리지 않아야 한다
- 긴 문장은 단어 단위로 자연스럽게 줄바꿈되어야 한다
- 키보드가 올라와도 입력칸, CTA, 오류 문구가 가려지지 않아야 한다
- 스크롤이 필요한 화면은 실제 스크롤이 가능하고, 사용자가 스크롤 가능한 구조임을 인지할 수 있어야 한다

### 짧은 화면 체크리스트

- 홈:
  - 인사 말풍선, 캐릭터, 바로가기, 메인 CTA가 지나치게 벌어지지 않아야 한다
  - 바로가기 카드는 높이가 먼저 줄고, 그다음 1열 전환을 고려한다
- 온보딩:
  - 캐릭터와 말풍선이 먼저 줄고, 제목은 2줄까지 허용한다
  - 마지막 페이지 CTA가 화면 밖으로 밀리지 않아야 한다
- 스몰토크:
  - 말풍선 최소 높이는 유지하되, 캐릭터와 하단 마이크 패널 높이를 줄여야 한다
  - 예시 답변이 한 단어씩 어색하게 끊기지 않아야 한다
- 플랫폼 확인:
  - 플랫폼 아이콘 영역이 먼저 줄고, 하단 진행 카드가 overflow 대신 내부 스크롤로 내려가야 한다
- 구매 이력:
  - 썸네일 카드는 1열 전환이 가능해야 하고, 카드 내부 텍스트가 이미지 위에서 읽혀야 한다
- 쇼핑 플로우:
  - 상단 단계바, 말풍선, 중앙 카드, 하단 마이크 패널이 동시에 보여질 때도 계층이 무너지지 않아야 한다
  - quantity / cart / password 단계에서 패널이 잘리지 않아야 한다
- 장바구니:
  - 핵심 요약은 먼저 보이되, 작은 화면에서는 전체 무스크롤보다 안전한 스크롤을 우선한다
- 로그인/회원가입:
  - 캐릭터가 먼저 줄고, 폼과 버튼은 키보드와 겹치지 않아야 한다

### 일반 화면 체크리스트

- 홈:
  - 캐릭터와 바로가기 사이 여백이 자연스럽고, CTA가 충분히 하단에 붙어 있어야 한다
- 온보딩:
  - 카드 안 캐릭터, 태그, 제목 비율이 과하게 비어 보이지 않아야 한다
- 스몰토크:
  - 말풍선, 캐릭터, 마이크 패널이 한 흐름으로 읽혀야 한다
- 플랫폼 확인 / 구매 이력:
  - 상단 설명 카드, 중앙 목록/카드, 하단 상태 영역이 서로 겹치지 않아야 한다
- 쇼핑 플로우:
  - 상품 카드가 하단 오버레이에 가리지 않고 주요 정보가 한눈에 보여야 한다
  - 단계별 전환 시 레이아웃 점프가 과하지 않아야 한다

### 태블릿 / 웹 넓은 화면 체크리스트

- 콘텐츠가 좌우 끝까지 늘어나지 않고 중앙 정렬 max width 안에 머물러야 한다
- 홈 바로가기, 구매이력 grid, 플랫폼 아이콘 목록은 넓은 공간을 활용하되 터치 타겟 크기는 유지해야 한다
- 단계바는 넓은 화면에서도 지나치게 퍼지지 않아야 한다
- 말풍선 텍스트 폭은 읽기 좋은 길이로 제한되어야 한다
- 웹에서 세로 길이가 충분해질 경우, 요소 사이 과도한 빈 공간이 생기지 않아야 한다
...
- standard 이상 화면에서는 핵심 정보가 최대한 1화면 안에 들어오고
- compact 화면에서는 overflow 대신 안전한 스크롤로 이어질 것
