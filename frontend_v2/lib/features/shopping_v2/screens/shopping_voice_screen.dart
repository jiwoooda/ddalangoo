...
class _ShoppingVoiceScreenState extends State<ShoppingVoiceScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final bottomOverlayReservedHeight =
        ...
        : 0.0;
    final bottomFloatingControlReservedHeight =
        !_textInputMode &&
            _controller.shouldShowVoiceButton &&
            _requiresBottomFloatingControlClearance(_controller.step)
        ? 176.0
        : 0.0;
    final useFakeGlass =
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: [
                        Padding(
                          padding: EdgeInsets.symmetric(
                            horizontal: horizontalPadding == 0 ? 20 : 0,
                          ),
                          child: const Align(
                        ...
                        Expanded(
                          child: Padding(
                            padding: EdgeInsets.only(
                              bottom:
                                  bottomOverlayReservedHeight +
                                  bottomFloatingControlReservedHeight,
                            ),
  ...
  bool _requiresBottomFloatingControlClearance(ShoppingStep step) {
    switch (step) {
      case ShoppingStep.cartCompleted:
      case ShoppingStep.askMoreOrCheckout:
        return true;
...
class _SegmentedTopTextState extends State<_SegmentedTopText> {
  ...
  List<String> _split(
    ...
    if (text.contains('\n')) {
      ...
      if (parts.length == 2 &&
          _fits(parts.first, style, width, height, maxLines) &&
          _fits(parts.last, style, width, height, maxLines)) {
        return parts;
      }
    }
    if (_fits(text, style, width, height, maxLines)) {
      return [text];
    }
    ...
    final candidates = <int>{
      for (int offset = 0; offset < words.length; offset++) ...[
        if (half + offset < words.length) half + offset,
        if (half - offset > 0) half - offset,
      ],
    }.toList();
    for (final split in candidates) {
      final part1 = words.sublist(0, split).join(' ');
      final part2 = words.sublist(split).join(' ');
      if (_fits(part1, style, width, height, maxLines) &&
          _fits(part2, style, width, height, maxLines)) {
        return [part1, part2];
      }
    }
    return [text];
  ...
  bool _fits(
    ...
    final extraSpacing =
        (lineCount > 1 ? lineCount - 1 : 0) * style.fontSize! * 0.38;
    final estimatedHeight = tp.height + extraSpacing;
