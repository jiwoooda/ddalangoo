...
enum class RuleReasonCode(val value: String) {
    ...
    PURCHASE_HISTORY_EXPAND("purchase_history_expand"),
    PURCHASE_HISTORY_PERIOD_FILTER_OPEN("purchase_history_period_filter_open"),
    PURCHASE_HISTORY_PERIOD_FILTER_SELECT("purchase_history_period_filter_select"),
...
class RuleBasedPlanner {
    ...
    private fun planMyKurly(filteredNodes: List<UiNode>): ActionPlan {
        val myKurlyNode = filteredNodes
            .filter { node -> containsAny(node, myKurlyKeywords) }
            .maxByOrNull { node ->
                var score = 0.0
                if (node.clickable || node.role == "button") score += 0.45
                if (node.centerY >= 1850) score += 0.35
                if (node.searchableText().trim() in myKurlyKeywords) score += 0.2
                score
            }
    ...
    private fun planOrderHistory(filteredNodes: List<UiNode>): ActionPlan {
        val orderHistoryNode = filteredNodes
            .filter { node -> containsAny(node, orderHistoryKeywords) }
            .maxByOrNull { node ->
                var score = 0.0
                if (node.clickable || node.role == "button") score += 0.45
                if (node.centerY in 500..1500) score += 0.25
                if (normalize(node.searchableText()) in orderHistoryKeywords.map { normalize(it) }) score += 0.2
                if (containsAny(node, listOf("주문 내역", "주문내역"))) score += 0.2
                score
            }
