...
async def get_addresses_db(db: AsyncSession, user_id: int) -> AddressListResponse:
    ...
    addrs = await address_repository.get_addresses_by_user_id_db(db, user_id)
    return AddressListResponse(userId=user_id, addresses=[_to_item(a) for a in addrs])

async def create_address_db(db: AsyncSession, user_id: int, req: AddressCreateRequest) -> AddressItem:
    if not await user_repository.get_user_by_id_db(db, user_id):
        raise HTTPException(status_code=404, detail={"category": "USER_ERROR", "code": "USER_NOT_FOUND", "message": "사용자를 찾을 수 없습니다."})
    data = {"user_id": user_id, "address_label": req.addressLabel, "recipient_name": req.recipientName,
            "recipient_phone": req.recipientPhone or "", "zip_code": req.zipCode, "address_line1": req.addressLine1,
            "address_line2": req.addressLine2, "delivery_request": req.deliveryRequest, "is_default": req.isDefault}
    addr = await address_repository.create_address_db(db, data)
    return _to_item(addr)

async def update_address_db(db: AsyncSession, user_id: int, address_id: int, req: AddressUpdateRequest) -> AddressItem:
    addr = await address_repository.get_address_by_id_db(db, address_id)
    ...
    updated = await address_repository.update_address_db(db, address_id, data)
    return _to_item(updated)

async def set_default_address_db(db: AsyncSession, user_id: int, address_id: int) -> AddressDefaultResponse:
    addr = await address_repository.get_address_by_id_db(db, address_id)
    if not addr or addr["user_id"] != user_id:
        raise HTTPException(status_code=404, detail={"category": "USER_ERROR", "code": "ADDRESS_NOT_FOUND", "message": "주소를 찾을 수 없습니다."})
    await address_repository.set_default_address_db(db, user_id, address_id)
    return AddressDefaultResponse(userId=user_id, defaultAddressId=address_id, message="기본 배송지가 변경되었습니다.")

async def delete_address_db(db: AsyncSession, user_id: int, address_id: int) -> AddressDeleteResponse:
    addr = await address_repository.get_address_by_id_db(db, address_id)
    if not addr or addr["user_id"] != user_id:
        raise HTTPException(status_code=404, detail={"category": "USER_ERROR", "code": "ADDRESS_NOT_FOUND", "message": "주소를 찾을 수 없습니다."})
    await address_repository.delete_address_db(db, address_id)
