...
class KurlyCredentials {
  const KurlyCredentials({required this.id, required this.password});
  ...
  static Future<KurlyCredentials?> _saveFromControllers(
    ...
    if (id.isEmpty || password.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('컬리 아이디와 비밀번호를 모두 입력해주세요.')));
      return null;
    }
...
class KurlyWebviewAutomation {
  ...
  Future<String> run({
    ...
    final targetUrl =
        ...
        'https://www.kurly.com';
    _debug(
      'run start targetUrl=$targetUrl, canonicalProductUrl=$canonicalProductUrl, quantity=$quantity',
    );
    ...
    if (canonicalProductUrl != null &&
        canonicalProductUrl.contains('/goods/')) {
  ...
  Future<bool> _attemptLogin() async {
    ...
    try {
      ...
      await controller.loadRequest(
        Uri.parse('https://www.kurly.com/member/login'),
      );
      await _waitForPageLoad();
      var dismissedAlreadyLoggedIn =
          await _dismissAlreadyLoggedInDialogIfNeeded();
      ...
      for (var attempt = 0; attempt < 3; attempt++) {
        ...
        _debug(
          'login success detector attempt=${attempt + 1} result=$loginSucceeded',
        );
        if (loginSucceeded ||
            dismissedAlreadyLoggedIn ||
  ...
  Future<bool> _addToCartFromSearchResults({
    ...
    if (quantity > 1) {
      for (var i = 1; i < quantity; i++) {
        ...
        ''');
        _debug(
          'search sheet quantity increment attempt=${i + 1} result=$quantityResult',
        );
