### Payment Agent (LLM 없음)

---

```powershell
def payment_flow(payment: PaymentState):
    """
    ...
    available_options = extract_available_options(page_state.page)
    ...
    if available_options and not selected_options:
        ...
        option_message = build_option_question(
            available_options,
            current_index=0,
        )
    ...
    if selected_options:
        ...
        option_result = apply_options_to_page(
            page_state.page,
            selected_options,
        )
    ...
    cart_result = add_to_cart_or_buy_now(
        page_state.page,
        quantity=quantity,
    )
    ...
    delivery_address = (
        payment.get("delivery_address")
        or checkout.delivery_address
    )
    ...
    if not payment.get("address_confirmed", False):
        ...
        confirm_message = format_address_confirm_message(
            delivery_address
        )
    ...
    # 실제 페이지에 주소 입력
    address_result = fill_delivery_address(
        page_state.page,
        delivery_address,
    )
    ...
    validation = revalidate_product_on_page(
        page_state.page
    )
    ...
    if validation.price_changed:
        ...
        update_checkout_price(
            checkout.id,
            validation.current_price,
        )

        changed_message = (
            f"가격이 {validation.current_price}원으로 변경됐어요. "
            f"계속 진행할까요?"
        )
    ...
    existing_order = find_order_by_idempotency_key(
        checkout.id
    )
    ...
    if existing_order:
        return wait_for_user_action(
            ...
            ui_command=open_webview(
                "payment",
                existing_order.payment_url,
            ),
        )
    ...
    payment_page = proceed_to_naverpay(
        page_state.page
    )
